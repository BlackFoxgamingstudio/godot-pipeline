# Developer Guide: Sovereign Godot Pipeline
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/godot-pipeline.git
cd godot-pipeline
...
```bash
export SBB_GODOT_BIN_PATH=your_value_here
export SBB_EXPORT_PRESETS=your_value_here
export SBB_ITCH_IO_TOKEN=your_value_here
export SBB_GUT_SCENE_PATH=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-godot-pipeline)
...
```bash
# Build
docker build -t sovereign-godot-pipeline:dev .

# Run with env file
docker run --env-file .env -p 8786:8786 sovereign-godot-pipeline:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-godot-pipeline
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8786`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-godot-pipeline",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8786` in use | `lsof -i :8786` | Kill PID or change `SERVICE_PORT` |
